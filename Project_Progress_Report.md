...
**Insight:** Heating is the dominant energy-consuming zone.
...
Used statistical threshold: `Threshold = Mean + (2 × Std Dev)`
...
**Result:** Energy spikes and daily patterns clearly identified.

---

## Milestones 2 & 3: Preprocessing & Synthetic Data
...
**Interpretation:** Since p-value > 0.05, the series is non-stationary.
...
**Observation:** Weekend consumption is slightly higher than weekdays.
...
**Observation:** Noticeable variation between weeks within January, indicating intra-month fluctuation rather than smooth monthly trend.
...
**Observation:** Slight upward trend across January

### 10. DISTRIBUTION ANALYSIS
**Statistical Measures:** Skewness: 0.4641 (slightly right-skewed), Kurtosis: -1.2595 (platykurtic)

**Observation:** Data approximately symmetric

### 11. FREQUENCY DOMAIN ANALYSIS (FFT)
Applied Fast Fourier Transform (FFT) to detect dominant periodic components.

**Result:** Dominant detected period ≈ 4321 hours

**Interpretation:** No strong short-term periodic cycles detected beyond overall dataset span. Confirms weak seasonality result from decomposition.
...
**Interpretation:** CUSUM revealed localized structural changes in early months but stability in later months. This confirms absence of strong long-term drift in the system.
...
**Precondition:** The original time series was found to be non-stationary based on ADF test. To satisfy the stationarity requirement for Granger causality analysis, first-order differencing was applied to the dataset.
...
**Observation:** The plot appears scattered without a clearly defined attractor structure. This suggests that the system does not exhibit strong deterministic low-dimensional dynamics based on the visuals.
...
**Observation:** The computed Lyapunov exponent is negative (approximately -1.63), indicating that nearby trajectories converge over time. This suggests that the system is stable and does not exhibit chaotic behavior, with low sensitivity to initial conditions.
...
**Observation:** The CSD plots were generated to visualize shared frequency components between signals.
...
**Observation:** The correlations between features were generally not close to ±1, indicating the absence of extremely strong linear relationships. This suggests that the features are not highly redundant, although moderate correlations may still exist.
...
**Observation:** All VIF values were close to 1 (below 5), indicating that multicollinearity is minimal and the features can be reliably used for modeling without significant redundancy.
...
**Observation:** Among all appliances, the AC exhibited the highest mutual information with total power, indicating it has the strongest influence on overall energy consumption. Other appliances showed comparatively lower dependency.
...
**Observation:** EWMA smoothing reduces short-term fluctuations and highlights the underlying trend more clearly.

---

## Milestone 4: Model Training (Anomaly Detection & Forecasting)

### 21. TIME-SERIES COMPLEXITY & ANOMALY DETECTION 

**21.1 Shannon Entropy**
- Shannon Entropy measures randomness in the time series.
- **Observation & Interpretation:** Computed by converting data into a probability distribution. Obtained value ≈ 3.52. Indicates moderate randomness in energy consumption. Suggests presence of both structure and variability.

**21.2 Hurst Exponent (R/S Analysis)**
- Measures long-term memory of the time series.
- **Observation & Interpretation:** Computed using Rescaled Range (R/S) method. Obtained value ≈ 0.86 (> 0.5). Indicates strong persistence (trend-following behavior). High/low consumption tends to continue over time.

**21.3 Cumulative Deviation Plot**
- Shows cumulative deviation from mean over time.
- **Observation & Interpretation:** Plot exhibits a smooth V-shaped structure. Indicates presence of long-term trends. Supports the high Hurst exponent result and suggests data is not purely random.

**21.4 One-Class SVM (Anomaly Detection)**
- Used for boundary-based anomaly detection.
- **Observation & Interpretation:** Trained only on normal data patterns to strictly prevent data leakage. Detects points outside boundary as anomalies. Artificial anomalies (~2%) were introduced for testing. Model successfully identifies abnormal energy usage.
...
### 22. BASELINE FORECASTING MODEL (LINEAR REGRESSION)
To establish a performance baseline without data leakage or lookahead bias, a Linear Regression model was trained on historical data.

- **Approach:** Used legitimate 1-hour lag features (predicting time *t* using time *t-1*).
- **Data Split:** Chronological split (80% Train, 20% Test) to respect time-series integrity.
- **Evaluation:**
  - **RMSE:** 985.38
  - **MAE:** 825.01
- **Observation:** Successfully established a solid, non-leaking mathematical benchmark for total power consumption.

### 23. ADVANCED FORECASTING MODEL (MULTIVARIATE LSTM)
To capture deep temporal dependencies, a Multivariate Long Short-Term Memory (LSTM) network was designed.

- **Feature Engineering:** Total power was combined with temporal features (Hour of day, Day of week).
- **Approach:** A 24-hour lookback sequence mapped to predict the consecutive 25th hour.
- **Architecture:** 2 LSTM layers (64 units each) combined with 0.1 Dropout to prevent overfitting.
- **Evaluation:**
  - **RMSE:** 813.42
  - **MAE:** 636.52
- **Result:** The complex LSTM architecture significantly outperformed the baseline linear regression, successfully learning the non-linear, temporal energy consumption patterns.
